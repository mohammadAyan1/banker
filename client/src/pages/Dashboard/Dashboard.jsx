...
const Dashboard = () => {
  ...
  useEffect(() => {
    setBankSummaryPage(1);
    setBankCasesPage(1);
  }, [selectedMonth, selectedBankView, searchText, selectedStatuses, selectedEngineers, selectedCities]);
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-7 gap-4 mb-6">
                {reports.map((report, key) => (
                  <div
                    key={key}
                    onClick={() => setActiveComponent(report.component)}
                    className={`group relative flex flex-col items-center text-center p-4 h-36 rounded-xl cursor-pointer transition-all duration-300 border shadow hover:shadow-lg ${activeComponent === report.component
                        ? "border-[#B5121B] bg-[#FFF4F4]"
                        : "border-gray-300 bg-white"
                      }`}
                  >
