...
// }
// Replace the existing print style block
// if (!document.getElementById("mnfl-print-style")) {
//     const s = document.createElement("style");
//     s.id = "mnfl-print-style";
//     s.textContent = `
...
if (!document.getElementById("mnfl-print-style")) {
    ...
    @media print {
      .no-print { display: none !important; }
      body * { visibility: hidden !important; }
      /* Override screen CSS on print area */
      #mnfl-print-area {
      ...
      table { page-break-inside: auto; }
      tr    { page-break-inside: avoid; page-break-after: auto; }
      #mnfl-print-area > table:last-child,
      #mnfl-print-area > div:last-child {
        page-break-after: avoid !important;
        margin-bottom: 0 !important;
      }
    }
...
function PrintArea({ form }) {
    ...
    // Google Maps embed URL – same as used in the live location section
    const mapEmbedUrl = hasMapLocation
        ? `https://maps.google.com/maps?q=${mapLocation.lat},${mapLocation.lng}&z=15&output=embed`
        : "";
    ...
    return (
        ...
        >
            ...
            <table style={{ ...ps, marginBottom: 6, border: "2px solid black" }}>
                <tbody>
                    <tr>
                        <img src="/public/assets/images/header1.jpg" alt="logo" />
                    </tr>
                </tbody>
            </table>
            ...
            {/* Map Section – uses the same Google Maps embed as the live view */}
            {hasMapLocation && (
                <div style={{ marginTop: 10 }}>
                    <div style={{ fontWeight: "bold", marginBottom: 5 }}>
                        Location Map
...
export default function ManappuramForm() {
    ...
    // Add this function inside ManappuramForm component
    ...
    //     const filename = `Valuation Report ${dd}-${mm}-${yyyy} ${hh}-${min}-${ss}.pdf`;

    //     // Clone and CHANGE the id so the screen CSS rule for
    //     // #mnfl-print-area (max-height:1px, opacity:0) does NOT apply
    //     const clone = source.cloneNode(true);
    //     clone.id = "mnfl-print-clone";          // ← key fix
    //     clone.removeAttribute("style");         // strip any leftover inline styles

    //     // Apply fully visible styles directly
    //     clone.style.cssText = [
    //         "position: static",
    //         "left: auto",
    //         "top: auto",
    ...
    //     // Mount clone in a transparent off-screen wrapper
    ...
    //     /* ────────────────────────────────────────────────────────────────────────
    //        FIX 1 — Replace every <iframe> (Google Maps embed) with a static <img>
    //        html2canvas cannot render iframes at all; we swap them out before capture.
    //     ─────────────────────────────────────────────────────────────────────────*/
    //     clone.querySelectorAll("iframe").forEach((iframe) => {
    //         const src = iframe.getAttribute("src") || "";
    //         // extract lat,lng from  ?q=LAT,LNG&...
    //         const m = src.match(/[?&]q=([^,&]+),([^&]+)/);
    ...
    //             // OpenStreetMap free static-map service (no API key required)
    //             mapImg.src = `https://staticmap.openstreetmap.de/staticmap.php?center=${lat},${lng}&zoom=15&size=600x220&markers=${lat},${lng},red-marker`;
    //             mapImg.style.cssText = "width:100%;height:220px;border:1px solid black;object-fit:cover;display:block;";

    //             iframe.parentNode.replaceChild(mapImg, iframe);
    //         } else {
    //             // unknown iframe — remove it
    //             iframe.remove();
    //         }
    //     });

    //     /* ────────────────────────────────────────────────────────────────────────
    //        FIX 2 — Convert every <img> to a base64 data-URL
    //        Cross-origin images cause html2canvas to produce blank boxes unless
    //        the server sends CORS headers. Converting to base64 sidesteps this.
    //     ─────────────────────────────────────────────────────────────────────────*/
    //     const toBase64 = (imgEl) =>
    //         new Promise((resolve) => {
    //             const src = imgEl.getAttribute("src");
    //             if (!src || src.startsWith("data:")) return resolve(); // already inline
    ...
    //                     reader.onload = () => {
    //                         imgEl.src = reader.result; // swap in data-URL
    //                         resolve();
    //                     };
    //                     reader.onerror = resolve;
    //                     reader.readAsDataURL(blob);
    //                 })
    //                 .catch(resolve); // on CORS failure leave src as-is (allowTaint handles it)
    //         });

    //     await Promise.allSettled(
    //         Array.from(clone.querySelectorAll("img")).map(toBase64)
    //     );

    //     /* ── mount off-screen wrapper ── */
    //     const wrapper = document.createElement("div");
    //     wrapper.style.cssText =
    ...
    //     // Give the browser a moment to paint the newly-set base64 sources
    ...
    //             scale: 2,
    //             useCORS: true,
    //             allowTaint: true,   // fallback if CORS headers missing on server
    //             logging: false,
    //             scrollX: 0,
    //             scrollY: 0,
    //             windowWidth: 794,
    //             imageTimeout: 20000, // extra time for the static-map image to load
    //         },
    //         jsPDF: {
    //             unit: "mm",
    //             format: "a4",
            ...
            <div className="p-4">
                ...
                <div className="border-2 border-black flex items-start p-2">
                    <div className="w-full text-center py-2">
                        <img src="/assets/images/header1.jpg" alt="Logo" />
                    </div>
                </div>
