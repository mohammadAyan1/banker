...
const LIVEABLE_FLOOR_OPTIONS = Array.from({ length: 15 }, (_, i) => ({ value: String(i + 1), label: String(i + 1) }));
const LB_LIVEABLE_OPTIONS = [{ value: "1", label: "1" }, { value: "2", label: "2" }];
...
/**
 * Returns floor label for a given key.
 * key formats: "basement_2", "parking_1", "ground", "1", "2", ...
 */
...
/**
 * Generates all floor keys based on basement levels, parking levels, liveable floors.
 * Order (top to bottom): Basement N..1, Parking N..1, Ground, 1st, 2nd...
 */
function getAllFloorKeys(basementLevels, parkingLevels, liveableFloors) {
  const b = parseInt(basementLevels) || 0;
  const p = parseInt(parkingLevels) || 0;
  const f = parseInt(liveableFloors) || 0;
  const keys = [];
  // Basements: highest to lowest
  for (let i = b; i >= 1; i--) keys.push(`basement_${i}`);
  // Parking: highest to lowest
  for (let i = p; i >= 1; i--) keys.push(`parking_${i}`);
  // Liveable: ground + upper floors
  if (f > 0) {
    keys.push("ground");
...
/**
 * Auto-computes structure configuration string.
 * Non-LB: "2B+2P+4" | "B+P+1" (1 = no prefix number)
 * LB: "G" | "G+1" | "G+3"
 */
...
const css = `
  ...
  .pf-wrap .pf-lb-badge { display: inline-block; margin-top: 3px; font-size: 11px; color: #1677ff; font-weight: 500; }

  /* Config badge — auto computed, blue pill */
  ...
  /* Floor key type colors in accordion */
  ...
  @media (max-width: 900px) {
    .pf-wrap .pf-occupancy-grid { grid-template-columns: 1fr; }
  }
`;
...
// Floor type classifier for styling
...
const PropertyDetailsForm = ({ data, editData, onSave, onSaveAndNext, saving }) => {
  ...
  // Selected floors for Floor Number checkboxes
  const [selectedFloors, setSelectedFloors] = useState([]);

  // Floor details: { [floorKey]: [unitObj, ...] }
  const [floorDetails, setFloorDetails] = useState({});
  // Currently open floor accordion in Floor Details
  const [openFloor, setOpenFloor] = useState(null);
  // Currently selected unit in Floor Details right panel
  const [selectedUnit, setSelectedUnit] = useState(null);

  // Area data: { [floorKey]: [{ id, unitNo, areas: [] }, ...] }
  ...
  // ── HANDLE CHANGE (with auto structure config) ─────────────────────────────
  const handleChange = useCallback((field, value) => {
    setForm((prev) => {
      const u = { ...prev, [field]: value };
      ...
      // Structure type cascade
      if (field === "typeOfStructure") {
        u.liveableFloors = ""; u.structureConfiguration = "";
        // Clear basement/parking if switching to non-basement structure
      ...
      // Fires whenever any of the 4 relevant fields change
      if (["typeOfStructure", "basementLevels", "parkingLevels", "liveableFloors"].includes(field)) {
        ...
        const isLB = structType === "load_bearing";
    ...
    try {
      ...
      if (json?.[0]?.Status === "Success" && json[0].PostOffice?.length) {
        ...
        setForm((p) => ({
          ...
          streetName: p.streetName || first.Name || "",
          landmark: p.landmark || first.Name || "",
        }));
        setPincodeOptions({
          talukas: uniq(offices.map((o) => o.Block || o.Division)),
  ...
  const isCommercial = subType === "commercial";
  const isPlot = unitType === "plot" && (isResidential || !!subType);
  const isLB = form.typeOfStructure === "load_bearing";
  const showStructuralFields = !isPlot;
  const showBasement = showStructuralFields && WITH_BASEMENT_PARKING.has(form.typeOfStructure);
  ...
  // All possible floor keys based on current form state
  const allFloorKeys = getAllFloorKeys(
    showBasement ? form.basementLevels : "",
    showBasement ? form.parkingLevels : "",
    form.liveableFloors
  );

  // When allFloorKeys changes, prune selectedFloors that no longer exist
  useEffect(() => {
    setSelectedFloors((prev) => {
      const valid = prev.filter((k) => allFloorKeys.includes(k));
      if (valid.length === prev.length) return prev;
      // Clean up floor details and area data for removed floors
      const removed = prev.filter((k) => !allFloorKeys.includes(k));
      if (removed.length > 0) {
        setFloorDetails((fd) => {
          const n = { ...fd };
          removed.forEach((k) => delete n[k]);
          return n;
        });
        setAreaData((ad) => {
          const n = { ...ad };
  ...
  const toggleFloor = (key) => {
    setSelectedFloors((prev) => {
      ...
      // Select — initialize empty arrays
  ...
  const handleFloorOpen = (fk) => {
    ...
    if (firstUnit) {
      setSelectedUnit({ floorKey: fk, unitId: firstUnit.id });
      setSelectedAreaUnit({ floorKey: fk, unitId: firstUnit.id });
      return;
    }
    setSelectedUnit(null);
    setSelectedAreaUnit(null);
  ...
  const handleOccupiedTypeChange = (value) => {
    ...
    if (value === "single") {
      setFloorDetails((fd) => {
        const next = {};
        Object.entries(fd).forEach(([fk, units]) => {
          next[fk] = (units || []).map((u) => ({
            ...u,
            occupiedBy: "",
            occupantName: "",
  ...
  // Check if floor number section is ready to show
  const canShowFloorNumbers = showStructuralFields && !!form.typeOfStructure && allFloorKeys.length > 0;
  const floorNumberNotReady = showStructuralFields && (
    !form.typeOfStructure
      ? "Select Type of Structure first"
      : allFloorKeys.length === 0
        ? "Set Livable Floors (and Basement/Parking levels) first"
        : null
  );
      ...
      <div className="pf-wrap">
        ...
        <div className="pf-card">
          ...
          <div className="pf-addr-row">
            <AF label="Landmark" required info>
              <Sel field="landmark" options={pincodeOptions?.landmarks || (form.landmark ? [{ value: form.landmark, label: form.landmark }] : [])} placeholder="Select Landmark" disabled={!pincodeOptions && !form.landmark} {...fp} />
            </AF>
        ...
        <div className="pf-card">
          ...
          <div className="pf-det-grid">
            ...
            {showBasement && (
              <DF label="Basement Levels" required info>
                <Sel field="basementLevels" options={[{value:"0",label:"0"},{value:"1",label:"1"},{value:"2",label:"2"},{value:"3",label:"3"}]} {...fp} />
              </DF>
            )}
            {showBasement && (
              <DF label="Parking Levels" required info>
                <Sel field="parkingLevels" options={[{value:"0",label:"0"},{value:"1",label:"1"},{value:"2",label:"2"}]} {...fp} />
              </DF>
            )}
            {showStructuralFields && (
              <DF label="Livable Floors" required info>
                <Sel field="liveableFloors" options={isLB ? LB_LIVEABLE_OPTIONS : LIVEABLE_FLOOR_OPTIONS} {...fp} />
              </DF>
            )}

            {/* Row 4 — Structure Configuration: always readonly, auto-computed */}
            {showStructuralFields && (
            ...
            {/* Floor Number — dynamic checkboxes: Basement N..1, Parking N..1, Ground, 1st, 2nd... */}
            {showStructuralFields && (
          ...
          <div>
            <Label>Door Photo With Name Plate</Label>
            <div className="pf-radio-group" style={{ marginTop: 4 }}>
              <label className="pf-radio"><input type="radio" name="doorPhotoWithNamePlate" checked={form.doorPhotoWithNamePlate === true} onChange={() => handleChange("doorPhotoWithNamePlate", true)} /> Yes</label>
              <label className="pf-radio"><input type="radio" name="doorPhotoWithNamePlate" checked={form.doorPhotoWithNamePlate === false} onChange={() => handleChange("doorPhotoWithNamePlate", false)} /> No</label>
            </div>
            {form.doorPhotoWithNamePlate === true && (
              <div>
                <div className="pf-upload-dragger" onClick={() => document.getElementById("doorPhotoInput").click()}>
                  <div className="pf-upload-icon">↑</div>
                  <div className="pf-upload-text">Drag &amp; Drop / Upload Photo *</div>
                  {form.doorPhotoFile && <div style={{ fontSize: 12, color: "#52c41a", marginTop: 4 }}>✓ {form.doorPhotoFile.name}</div>}
                </div>
