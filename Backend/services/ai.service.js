...
- If the original text is in Hindi, translate it to English.
- Preserve numbers, proper nouns (names, places) as they appear if they are already in Roman script, but ensure they are in English.
...
Rules:
- All text must be in English (translate Hindi if needed).
- If a field is not found, return null.
- Numbers can stay as numbers or English.
- Registration date format: DD/MM/YYYY
- Latitude and longitude, if present, should be extracted as strings (e.g., "26.285941").
- If the data comes from Excel/CSV, look at column headers and row values carefully to map to the above JSON structure.
- If the file is a valuation report, use aliases:
  - "Name of Property Owner", "Owner Name" -> property.owner_name
  - "Applicant Name", "Name of Client", "Customer Name" -> property.applicant_name
  - "Contacted Person", "During Property Visit", "Person Met During Visit" -> property.contact_person
  ...
  - "Case Ref No.", "Case Reference Number", "Loan Ref No." -> registration_number when registry number is unavailable
  - "Date of Report" -> registration_date when registry date is unavailable
